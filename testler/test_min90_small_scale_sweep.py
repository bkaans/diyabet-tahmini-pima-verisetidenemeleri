...
from makine_ogrenmesi.kaynak.min90_small_scale_sweep import min90_small_scale_sweep_calistir


def test_min90_small_scale_quick_csv_json_ve_leakage_uretmelidir(tmp_path: Path) -> None:
    ...
    veri_dir = tmp_path / "makine_ogrenmesi" / "veri" / "deneysel" / "min90_small_scale"
    assert (rapor_dir / "min90_small_scale_sweep_results.json").exists()
    assert (rapor_dir / "min90_small_scale_selection_report.json").exists()
    assert (rapor_dir / "min90_small_scale_quality_audit.json").exists()

    csv_yollari = sorted(veri_dir.glob("small_t*_*.csv"))
    metadata_yollari = sorted(veri_dir.glob("small_t*_*_metadata.csv"))
    assert csv_yollari
