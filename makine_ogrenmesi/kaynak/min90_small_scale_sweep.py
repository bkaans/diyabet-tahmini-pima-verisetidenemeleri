...
class SweepConfig:
    ...
    def key(self) -> str:
        ...
        return f"small_t{self.target_per_class}_s{strength}_{self.noise_profile}"


def min90_small_scale_sweep_calistir(
    *,
    proje_koku: Path,
    veri_yolu: Path,
    ...
    veri_dir = proje_koku / "makine_ogrenmesi" / "veri" / "deneysel" / "min90_small_scale"
    grafik_dir = rapor_dir / "grafikler_min90_small_scale"
    ...
    _json_write(rapor_dir / "min90_small_scale_sweep_results.json", sweep_results)
    _json_write(rapor_dir / "min90_small_scale_selection_report.json", selection_report)
    _json_write(rapor_dir / "min90_small_scale_quality_audit.json", audit)
    ...
    if word_raporu_yaz:
        ...
        desktop_word_path = Path("/Users/beratkaanseven/Desktop/Min90_Kucuk_Olcek_Literatur_Stili_Rapor.docx")
...
def _build_quality_audit(raw: pd.DataFrame, veri_yolu: Path, sweep: dict[str, Any], selection: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            ...
            "Tüm aday CSV dosyaları min90_small_scale klasörüne yazıldı.",
...
def _plot_confusion_matrix(grafik_dir: Path, selection: dict[str, Any]) -> Path:
    path = grafik_dir / "small_scale_final_confusion_matrix.png"
...
def _plot_feature_signal(grafik_dir: Path, selection: dict[str, Any]) -> Path:
    path = grafik_dir / "small_scale_final_feature_signal.png"
...
def _write_word_report(*, proje_koku: Path, sweep: dict[str, Any], selection: dict[str, Any], graphics: dict[str, str]) -> Path:
    ...
    path = proje_koku / "makine_ogrenmesi" / "raporlar" / "min90_small_scale_literatur_stili_rapor.docx"
    doc = Document()
...
def _config_from_name(name: str) -> SweepConfig:
    ...
    strength = float(parts[2][1:].replace("p", "."))
    noise = "_".join(parts[3:])
