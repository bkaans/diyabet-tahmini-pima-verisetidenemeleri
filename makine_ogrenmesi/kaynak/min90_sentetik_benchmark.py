...
def min90_benchmark_calistir(
    *,
    ...
    """Min-0.90 hedefli sentetik benchmark aramasini calistirir."""
    ...
    print("[min90] hizli tarama basladi")
        ...
        for feature_spec in feature_specs:
            for model_spec in model_specs:
                ...
                try:
                    ...
                    print(
                        f"[min90/{dataset.name}/{feature_spec.name}/{model_spec.name}] "
                    ...
                    print(f"[min90/{dataset.name}/{feature_spec.name}/{model_spec.name}] hata: {exc}")

    successful = [r for r in holdout_results if r.get("status") == "completed"]
    if not successful:
        raise RuntimeError("Min90 benchmark icin basarili sonuc uretilemedi.")
    ...
    for candidate in cv_candidates:
        ...
        s = cv["summary"]
        print(
            f"[min90-cv/{cv['dataset_name']}/{cv['feature_set']}/{cv['model']}] "
    ...
    _json_write(rapor_dir / "min90_final_optimizasyon_sonuclari.json", optimization_report)
    _json_write(rapor_dir / "min90_final_model_secim_raporu.json", model_selection_report)
    _json_write(rapor_dir / "min90_final_leakage_raporu.json", leakage_json)
    _json_write(rapor_dir / "min90_final_dataset_karsilastirma.json", comparison_report)
    _json_write(rapor_dir / "min90_final_literatur_notlari.json", literature_notes)
    ...
    if word_raporu_yaz:
        ...
        desktop_word_path = Path("/Users/beratkaanseven/Desktop/PIMA_Sentetik_Benchmark_Min90_Final_Rapor.docx")
...
def _generate_source_safe_dataset(dev: pd.DataFrame, target_per_class: int, random_state: int) -> DatasetCandidate:
    ...
    metadata.insert(0, "row_id", [f"min90_r_{i:06d}" for i in range(len(metadata))])
    _validate_metadata(metadata)
    report = {
        "dataset_name": f"min90_sentetik_{target_per_class}_per_class_pima",
    ...
    return DatasetCandidate(
        name=f"min90_sentetik_{target_per_class}_per_class_pima",
...
def _plot_original_distribution(grafik_dir: Path, raw: pd.DataFrame) -> Path:
    path = grafik_dir / "min90_orijinal_pima_sinif_dagilimi.png"
...
def _plot_candidate_distribution(grafik_dir: Path, datasets: list[DatasetCandidate]) -> Path:
    path = grafik_dir / "min90_aday_veri_boyutlari.png"
...
def _plot_holdout_metric(grafik_dir: Path, comparison: dict[str, Any], metric: str, title: str) -> Path:
    path = grafik_dir / f"min90_holdout_{metric}.png"
...
def _plot_cv_metric(grafik_dir: Path, comparison: dict[str, Any], metric: str, title: str) -> Path:
    path = grafik_dir / f"min90_cv_{metric}.png"
    cv_by_dataset = {}
...
def _plot_threshold_effect(grafik_dir: Path, comparison: dict[str, Any]) -> Path:
    path = grafik_dir / "min90_threshold_tuning_etkisi.png"
    rows = _dataset_metric_rows(comparison)
    labels = [r[0] for r in rows]
...
def _plot_confusion_matrix(grafik_dir: Path, final: dict[str, Any]) -> Path:
    path = grafik_dir / "min90_final_confusion_matrix.png"
...
def _plot_feature_importance(grafik_dir: Path, final: dict[str, Any]) -> Path:
    path = grafik_dir / "min90_final_feature_importance.png"
    model = final["_model"]
...
def _plot_calibration(grafik_dir: Path, final: dict[str, Any]) -> Path:
    path = grafik_dir / "min90_final_calibration_curve.png"
...
def _plot_brier(grafik_dir: Path, comparison: dict[str, Any]) -> Path:
    path = grafik_dir / "min90_brier_karsilastirma.png"
    rows = _dataset_metric_rows(comparison)
    labels = [r[0] for r in rows]
...
def _plot_leakage(grafik_dir: Path, leakage_report: list[dict[str, Any]]) -> Path:
    path = grafik_dir / "min90_leakage_ozeti.png"
...
def _write_word_report(
    *,
    ...
    path = proje_koku / "makine_ogrenmesi" / "raporlar" / "pima_sentetik_benchmark_min90_final_rapor.docx"
    doc = Document()
