"""Mevcut min90 sonuçlarından açıklayıcı V2 Word raporu üretir."""
...
GRAFIK_DIR = RAPOR_DIR / "grafikler_min90_final"
GRAFIK_V2_DIR = RAPOR_DIR / "grafikler_min90_final_v2"
DOCX_PATH = RAPOR_DIR / "pima_sentetik_benchmark_min90_final_rapor_v2.docx"
DESKTOP_DOCX_PATH = Path("/Users/beratkaanseven/Desktop/PIMA_Sentetik_Benchmark_Min90_Final_Rapor_v2.docx")
AUDIT_PATH = RAPOR_DIR / "min90_final_quality_audit_v2.json"
VERI_DIR = PROJE_KOKU / "makine_ogrenmesi" / "veri" / "deneysel"


REQUIRED_GRAPHS = {
    "original_class_distribution": "min90_orijinal_pima_sinif_dagilimi.png",
    "candidate_distribution": "min90_aday_veri_boyutlari.png",
    "holdout_min_metric": "min90_holdout_ana_metrik_minimumu.png",
    "cv_min_metric": "min90_cv_ana_metrik_minimumu_mean.png",
    "holdout_roc_auc": "min90_holdout_roc_auc.png",
    "cv_roc_auc": "min90_cv_roc_auc_mean.png",
    "confusion_matrix": "min90_final_confusion_matrix.png",
    "feature_importance": "min90_final_feature_importance.png",
    "calibration_curve": "min90_final_calibration_curve.png",
    "brier": "min90_brier_karsilastirma.png",
    "leakage": "min90_leakage_ozeti.png",
}
...
def main() -> None:
    model_report = _json_load(RAPOR_DIR / "min90_final_model_secim_raporu.json")
    leakage_report = _json_load(RAPOR_DIR / "min90_final_leakage_raporu.json")
    dataset_report = _json_load(RAPOR_DIR / "min90_final_dataset_karsilastirma.json")
    fark_report = _json_load(RAPOR_DIR / "min90_vs_strict2700_fark_analizi.json")
...
def _prepare_graphs(dataset_report: dict[str, Any], fark_report: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    diff_graph = GRAFIK_V2_DIR / "min90_v2_strict2700_cross_model.png"
    shift_graph = GRAFIK_V2_DIR / "min90_v2_distribution_shift_cohens_d.png"
    feature_signal_graph = GRAFIK_V2_DIR / "min90_v2_feature_signal_importance.png"
    ...
    for key, path in {
        "strict_min90_cross_model": diff_graph,
...
def _plot_cross_model_graph(fark: dict[str, Any], path: Path) -> None:
    ...
    order = [
        ...
        ("strict_xgb_no_skin_on_min90_2500", "Yeni üretim\nXGBoost"),
        ("new_soft_all_on_strict2700", "Önceki üretim\nSoft voting"),
        ("new_soft_all_on_min90_2500", "Yeni üretim\nSoft voting"),
    ]
...
def _plot_shift_graph(fark: dict[str, Any], path: Path) -> None:
    ...
    dist_keys = ["original_dev", "strict2700_all_rows", "min90_2500_all_rows", "external_holdout"]
    cohen = [dist[key]["class_separation"]["aggregate"]["mean_abs_cohens_d"] for key in dist_keys]
    shifts = [0.0, shift["strict2700_all_rows"]["aggregate"]["avg_abs_z_mean_shift"], shift["min90_2500_all_rows"]["aggregate"]["avg_abs_z_mean_shift"], shift["external_holdout"]["aggregate"]["avg_abs_z_mean_shift"]]
...
def _plot_feature_signal_importance(path: Path) -> None:
    df = pd.read_csv(VERI_DIR / "min90_sentetik_2500_per_class_pima.csv")
...
def _build_quality_audit(
    ...
    return {
        ...
        "min90_vs_strict2700_summary": fark_report["decomposition"],
...
def _outcome_encoding_check() -> dict[str, Any]:
    paths = [
        ...
        VERI_DIR / "min90_sentetik_2500_per_class_pima.csv",
...
def _dataset_label(value: str) -> str:
    labels = {
        "min90_sentetik_2500_per_class_pima": "Kontrollü sentetik PIMA benchmarkı (2500/2500)",
        "min90_sentetik_2700_per_class_pima": "Kontrollü sentetik PIMA benchmarkı (2700/2700)",
        "min90_sentetik_3000_per_class_pima": "Kontrollü sentetik PIMA benchmarkı (3000/3000)",
        "min90_sentetik_4000_per_class_pima": "Kontrollü sentetik PIMA benchmarkı (4000/4000)",
        "min90_sentetik_5000_per_class_pima": "Kontrollü sentetik PIMA benchmarkı (5000/5000)",
        "min90_sentetik_7500_per_class_pima": "Kontrollü sentetik PIMA benchmarkı (7500/7500)",
        "min90_sentetik_10000_per_class_pima": "Genişletilmiş kontrollü sentetik PIMA benchmarkı (10000/10000)",
    }
...
def _write_docx(
    ...
    _img(doc, graphs.get("strict_min90_cross_model"), 5.9, "Grafik, aynı model farklı veri üzerinde denenince asıl artışın yeni sentetik üretim stratejisinden geldiğini göstermektedir.")
